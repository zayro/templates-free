...
class templateNewsy.mvcgallery.NetworkView extends AbstractView {
	
	////////////////////config news
	var __thumbLength:Number=3
	...
	var __scroll:Scroll
	var __mask2:MovieClip
	var __scrollValue:Number
...
function onMouseWheel(delta_) {
	
	if (this.__mask2.hitTest(_root._xmouse, _root._ymouse, true)) {
				this.onEnterFrame = Delegate2.create(this, onChangeScroll_oef)
		if (delta_ < 0) {
			__scroll.setScrollPosition(__scroll.getScrollPosition()-30)
		}else {
			__scroll.setScrollPosition(__scroll.getScrollPosition()+30)	
		}
	}
...
function addScroll() {
	...
	if(hasPortion()){
	__scroll = Scroll(this.attachMovie("_scroll", "_scroll_", 789))
	__scroll.setColorScrollBackground(Configuration.BACKGROUND_SCROLL_COLOR)
	__scroll.setColorScroll(Configuration.SCROLL_COLOR)
	
	__scroll.height = getHeightNetwork()
	__scroll.x = getWidthNetwork()+20
	__scroll.y =__scroll.spaceArrowY
    var proportion=getHeightNetwork()/__content._height
	__scroll.limit = [1,-__content._height+__mask._height,proportion];
	__scroll.onChange=Delegate.create(this,onChangeScroll)
	}
...
function onChangeScroll(value_:Number) {
	
	__scrollValue = value_
	
	if(this.onEnterFrame==undefined||onEnterFrame==null){
	this.onEnterFrame = Delegate2.create(this, onChangeScroll_oef)
	}
}
...
function onChangeScroll_oef() {
	this.__content._y += ( __scrollValue	 -this.__content._y) /4
...
function onResize(){
	...
	var widthNetwork:Number=m.width-150   /////area width network
	var heightNetwork:Number=m.height-200 ////area height network
	if(this.__orient=="H"){
	this.__maxRows=5//Math.floor(  (widthNetwork/(ThumbView.getWidth()+this.__spaceX)) )
	this.__maxColumns=Math.ceil(  (heightNetwork/(ThumbView.getHeight()+this.__spaceY)) )
	}else{
	this.__maxColumns=1//Math.round(  (widthNetwork/(ThumbView.getWidth()+this.__spaceX)) )
	this.__maxRows=__thumbLength//Math.ceil(  (heightNetwork/(ThumbView.getHeight()+this.__spaceY)) )
	}
    }	
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function hide() {
		...
		tweenAlpha.stop()
		tweenAlpha = new Tween(this, '_alpha', Strong.easeOut, this._alpha, 0, 1, 1)
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function prev(){
		...
		if(this.__motion==false&&this.__currentRows>=0){
		this.__currentRows-=this.__step
		changeY()
		}
	}
...
function changeY(){
		...
		__scroll.setScrollPosition(value,false)
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function create_mask(boolean_:Boolean){
		...
		__mask2 = __mask.duplicateMovieClip("mask2", 43543)
		__mask2._visible=false
