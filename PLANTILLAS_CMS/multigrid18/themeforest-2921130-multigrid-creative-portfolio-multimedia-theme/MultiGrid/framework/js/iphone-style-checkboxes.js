...
$.extend($[iphoneStyle].prototype, {
  ...
  wrapCheckboxWithDivs: function() {
    ...
    this.offLabel  = $('<label style="display: block"  class="'+ this.labelOffClass +'">' +
  ...
  optionallyResize: function(mode) {
      //alert("we"+$(this.container).children('.iPhoneCheckLabelOff').outerWidth());
      //$(this).css('width');

    var onLabelWidth  = this.onLabel.width(),
        offLabelWidth = this.offLabel.width();
        if(this.defaultOnWidth!=0){
            onLabelWidth=this.defaultOnWidth;
        }
        if(this.defaultOffWidth!=0){
            offLabelWidth=this.defaultOffWidth;
        }
//alert(this.handle.width());
    var newWidth=0;
    if (mode == 'container') {
      newWidth = (onLabelWidth > offLabelWidth) ? onLabelWidth : offLabelWidth;
      newWidth += (this.handleWidth + 15);
      this.containerWidth=newWidth;
    } else {
      newWidth = (onLabelWidth < offLabelWidth) ? onLabelWidth : offLabelWidth;
      //alert($(this.onLabel).width());
    }
    if(mode=='handle'){
        this.handleWidth=newWidth;
    }
   // alert(newWidth);
    $(this[mode]).width(newWidth);//({width: newWidth});
  },
  ...
  initialPosition: function() {
    this.offLabel.css({width: this.containerWidth - 5});

    var offset = ($.browser.msie && $.browser.version < 7) ? 3 : 6;
    this.rightSide = this.containerWidth - this.handleWidth - offset;
