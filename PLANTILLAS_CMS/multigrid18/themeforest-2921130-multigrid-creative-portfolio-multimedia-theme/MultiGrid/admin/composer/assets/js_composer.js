...
jQuery(document).ready(function($) {
	...
	*/
	...
	if ( $("#wpb_visual_composer").length == 1 ) {
		$('div#titlediv').after('<p class="composer-switch"><a class="wpb_switch-to-composer button-primary" href="#">Theme Elements</a></p>');
...
function ttSave(element) {
    ...
    switch(element.attr('data-element_type')){
        case"tt_table":{
            ...
            switch(class_to_set){
                case'default_table':   class_to_set="table";                               break;
                case'striped_table':   class_to_set="table table-striped";                               break;
                case'bordered_table':  class_to_set="table table-bordered";                              break;
                case'condensed_table': class_to_set="table table-condensed";                             break;
                case'combine_them_all':class_to_set="table table-striped table-bordered table-condensed";break;
            }
        ...
        case"tt_progress":{
            ...
            class_to_set = el.val();
            switch(class_to_set){
                case'basic':   class_to_set=" progress";                        break;
                case'striped': class_to_set=" progress progress-striped";       break;
                case'animated':class_to_set=" progress progress-striped active";break;
            }
...
---------------------------------------------------------- */
function wpbTestimonialsInitCallBack(element) {
	element.find('.wpb_tabs_holder').not('.wpb_initialized').each(function(index) {
		jQuery(this).addClass('wpb_initialized');
		//var tab_counter = 4;
		//
		...
		delete_btn.click( function(e) {
			...
			var answer = confirm ('Press OK to delete "'+tab_name+'" testimonial, Cancel to leave');
			if ( answer ) {
				var selected = $tabs.tabs('option', 'selected');
				$tabs.tabs( "remove", selected );
				//
		...
		add_btn.click( function(e) {
			e.preventDefault();
			var tab_title = ( jQuery(this).closest('.wpb_sortable').hasClass('wpb_vc_tour')) ? 'Slide' : 'Testimonial',
				tabs_count = $tabs.tabs( "length" ) + 1;
			$tabs.tabs( "add", "#tabs-" + tabs_count, tab_title );
			//tab_counter++;
			//
		...
		edit_btn.click( function() {
			...
			var tab_title = prompt("Please enter new testimonial title", tab_name);
			if ( tab_title != null && tab_title != "" ) {
				$tabs.find('ul li.ui-tabs-selected a').text(tab_title);
				//
				save_composer_html();
...
function wpbTestimonialsGenerateShortcodeCallBack(current_top_level, inner_element_count) {
	var tab_title = current_top_level.find(".ui-tabs-nav li:eq("+inner_element_count+") a").text();
	output = '[tt_testimonial title="'+tab_title+'"] %inner_shortcodes [/tt_testimonial]';
	return output;
}
