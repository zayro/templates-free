...
function yiw_sc_section_func($atts, $content = null) 
{
    ...
    $html .= "    $img\n";
    $html .= "    {$before_title}<span style=\"line-height:{$size}px\">{$title}</span>{$after_title}"; 
...
function yiw_sc_one_fourth_func($atts, $content = null) {        
	...
	$html = "<div class=\"" . implode( $classes, ' ' ) . "\">".wpautop( do_shortcode( $content ) )."</div>";
	
	return apply_filters( 'yiw_sc_onw_fourth_html', $html );
}     
endif;
