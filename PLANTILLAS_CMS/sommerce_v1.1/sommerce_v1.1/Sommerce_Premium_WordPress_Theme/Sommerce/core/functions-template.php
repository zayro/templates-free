...
function yiw_layout_page()
{
    ...
    return ( isset($post->ID) &&  ($layout=get_post_meta( $post->ID, '_layout_page', true )) ) ? $layout : YIW_DEFAULT_LAYOUT_PAGE;
}              
...
function yiw_get_img( $relative_path, $alt = '', $class = '' ) {
    if ( !file_exists( YIW_THEME_PATH . '/' . $relative_path ) )
        return;
