	...
	function theme_register_cufon() {
		...
		echo "<script type='text/javascript'>
				...
				if ((get_option('default_theme_color')) == 'dark'):
					echo "Cufon.replace('h1, h2, h3, h4', { fontFamily: '{$font_name}', hover: false, color: '-linear-gradient(#ffffff, #bababa)' });";
				else:
					echo "Cufon.replace('h1, h2, h3, h4', { fontFamily: '{$font_name}', hover: false, color: '-linear-gradient(#727171, #020202)' });";
				endif;
				
				echo "Cufon.replace('.kwicks .slider_title', {fontFamily: '{$font_name}', hover: false, color: '-linear-gradient(#ffffff, #bababa)' });
				Cufon.replace('.big_btn', { fontFamily: '{$font_name}', hover: true });
				Cufon.replace('#fsb h3', { fontFamily: '{$font_name}', hover: true, textShadow: '1px 1px rgba(0, 0, 0, 0.8)', color: '-linear-gradient(#FCFCFC, #B8B8B8)' });
				Cufon.replace('#fsb h4, .gallery h4, .partners h4, .header_full h2, .header_full h3, .overview h4', { fontFamily: '{$font_name}', hover: true, color: '-linear-gradient(#ffffff, #bababa)' });
				Cufon.now();
			});
		</script>";
	}
