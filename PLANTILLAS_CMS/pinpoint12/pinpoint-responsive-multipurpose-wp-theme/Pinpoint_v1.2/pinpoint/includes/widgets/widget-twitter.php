			...
			if (method_exists($feed, 'get_items')) :
				...
				foreach	($tweets as $t) :
					...
					if ((abs(time() - $time)) < 86400) :
						$time = human_time_diff($time) . ' ' . sprintf(__('ago', 'swiftframework'));
					else :
						$time = date(($date), $time);
					endif;
			    	...
			    	// Match @name
			    	$text = preg_replace('/(@)([a-zA-Z0-9\_]+)/', '@<a href="http://twitter.com/$2" target="_blank">$2</a>', $text);
			    	
			    	// Match #tag
			    	$text = preg_replace('/(#)([a-zA-Z]+)/', '<a href="http://twitter.com/search/%23$2" target="_blank">#$2</a>', $text);
					endif;

		    		// Display message without username prefix
					$prefixlen = strlen($username . ": ");
					$result .= '
							<div class="twitter-text">' . substr($text, $prefixlen, strlen($text) - $prefixlen) . '</div>';
