...
class WPBakeryShortCode_VC_Gmaps extends WPBakeryShortCode {
    ...
    protected function content( $atts, $content = null ) {
        ...
        $output .= "\n\t\t".'<div class="wpb_wrapper">';
        $output .= ($title != '' ) ? "\n\t\t\t".'<h3 class="wpb_heading wpb_video_heading">'.$title.'</h3>' : '';
        $output .= '<div class="wpb_map_wraper"><div id="map-canvas" style="width:100%;height:'.$size.'px;" data-address="'.$address.'" data-lat="'.$map_coords['lat'].'" data-long="'.$map_coords['long'].'" data-zoom="'.$zoom.'" data-maptype="'.$type.'" data-pinimage="'.$img_url[0].'"></div></div>';
        $output .= "\n\t\t".'</div> '.$this->endBlockComment('.wpb_wrapper');
        $output .= "\n\t".'</div> '.$this->endBlockComment($width);
