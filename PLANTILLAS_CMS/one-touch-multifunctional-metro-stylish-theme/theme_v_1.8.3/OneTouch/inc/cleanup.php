...
class Roots_Alt_Walker extends Walker_Nav_Menu
{
    ...
    function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
    {
        ...
        $location_id = 'primary_navigation';
        ...
        $output .= $indent . '<li' . $class_names . '><span class="menu-item-wrap">';
        ...
        if(isset($menu_slug)){
            $styles = (array)get_option("custom_metro_menus");
            if(isset($styles[$menu_slug]['items'][$item->ID])){
                $styles = $styles[$menu_slug]['items'][$item->ID];
                if(!aq_resize( $styles['bgimage'], 140, 140,true))
                    $background_image = 'none';
                else
                    $background_image = 'url('.aq_resize( $styles['bgimage'], 140, 140,true).')';

                $attributes .= " style='background-color:".$styles['color']."; background-size:cover; background-image:".$background_image.";'";
                $icon = $styles['icon'];
            }
        }
        ...
        $page  = get_post_meta( $item->object_id, '_wp_page_template', true );
        if ( ($page == 'page-contacts.php') && ( $NHP_Options->get('faces_on_contact_menu') ) ){
            $args_children = array( 'post_parent' => (int)$item->object_id, 'post_type' => 'page', );
            $children = get_posts($args_children);
            $contact_icons = "";
            $contact_icon_counter = 0;
            foreach($children as $child){
                if (has_post_thumbnail($child->ID)) {
                    $thumb = get_post_thumbnail_id($child->ID);
                    $contact_icon_counter++;
                    $img_url = wp_get_attachment_url($thumb, 'full'); //get img URL
                    if($contact_icon_counter == 1){
                        $img_url = aq_resize($img_url, 92,92,true);
                    } else if($contact_icon_counter == 2){
                        $img_url = aq_resize($img_url, 92, 46, true);
                    } else if($contact_icon_counter == 3){
                        $img_url = aq_resize($img_url, 46, 46, true);
                    }
                    $contact_icons .= "<img src='".$img_url."' class='contact-menu-icons contact-icon-".$contact_icon_counter."' alt='' />";
                }
            }
        }
        ...
        $item_output .= isset($contact_icons)?$contact_icons:'';
        $item_output .= '<span class="link-text">' . $args->link_before . apply_filters('the_title', $item->title, $item->ID) . $args->link_after . '</span>';
        $item_output .= ($args->has_children) ? '<span class="arrow">&nbsp;</span>' : '';
        if(isset($icon)){
            $item_output .= "<span class='tile-icon' style='background-image:url(".aq_resize( $icon, 96, 96,true).");'></span>";
        };
