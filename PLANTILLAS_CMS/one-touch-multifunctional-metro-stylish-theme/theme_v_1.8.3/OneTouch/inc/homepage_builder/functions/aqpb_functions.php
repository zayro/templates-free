...
if(class_exists('AQ_Page_Builder')) {
	...
	********************************************************/
    ...
    function aq_field_hide_element_checkbox($field_id, $block_id, $check, $hide = array()) {

        $jQuery_selectors_to_hide = '';
        foreach($hide as $id){
            $jQuery_selectors_to_hide .= 'label[for='.$id."],";
        }

        $jQuery_selectors_to_hide = substr($jQuery_selectors_to_hide,0, strlen($jQuery_selectors_to_hide) - 1);
        $output = '<input type="hidden" name="aq_blocks['.$block_id.']['.$field_id.']" value="0" />';
        $output .= '<input type="checkbox"  id="'. $block_id .'_'.$field_id.'" onchange="( jQuery(this).attr(\'checked\') )?(jQuery(\''.$jQuery_selectors_to_hide.'\').show()):jQuery(\''.$jQuery_selectors_to_hide.'\').hide();" class="input-checkbox" name="aq_blocks['.$block_id.']['.$field_id.']" '. checked( 1, $check, false ) .' value="1"/>';
